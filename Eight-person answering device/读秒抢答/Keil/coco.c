...
uchar P3is; //记录一瞬间的按键
uchar num_seg[12] = {0X3F,0X06,0X5B,0X4F,0X66,0X6D,0X7D,0X07,0X7F,0X6F,0X77,0X7C};  //数码管共阴极字段码
uchar disp_now = 0X88;
	...
	while(1)
	{
		if(!ctrl)  // 主持人按下按钮进入抢答
		{
			...
			while(1)	
			{
				...
				if(P3 != 0XFF && disp_now == 0X88) //某个按钮按下时进入，检测哪个按钮被按下,我的意思是一次循环那么快，就算两个按钮同时按下，也有先后顺序吧，也能检测到吧（Doge）
				{
					P3is = P3;
